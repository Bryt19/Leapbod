...
export default function OpportunitiesPage() {
  ...
  const [sortBy, setSortBy] = useState<'newest' | 'deadline' | 'featured'>('newest')
  ...
  useEffect(() => {
    console.log('OpportunitiesPage useEffect triggered. Auth loading:', authLoading, 'User:', user?.id)
  ...
  const fetchBookmarks = async () => {
    ...
    try {
      ...
      setBookmarkedOpportunities(data?.map(bookmark => bookmark.opportunity_id) || [])
    } catch (error) {
      console.error('Error fetching bookmarks:', error)
          ...
          <div className="text-center space-y-6">
            <div className="space-y-4">
              ...
              <p className="text-xl sm:text-2xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
                Find internships, scholarships, competitions, and more opportunities 
                <span className="font-semibold text-foreground">tailored for students like you</span>
              </p>
            </div>
            
            <div className="flex flex-wrap justify-center gap-3 pt-6">
              <Badge variant="secondary" className="bg-blue-50 text-blue-700 border-blue-200 px-4 py-2 text-sm font-medium">
                🎓 Student-focused
              </Badge>
              <Badge variant="secondary" className="bg-green-50 text-green-700 border-green-200 px-4 py-2 text-sm font-medium">
                🔍 Carefully curated
              </Badge>
              <Badge variant="secondary" className="bg-purple-50 text-purple-700 border-purple-200 px-4 py-2 text-sm font-medium">
                🚀 Career-boosting
              </Badge>
              <Badge variant="secondary" className="bg-orange-50 text-orange-700 border-orange-200 px-4 py-2 text-sm font-medium">
                ⚡ Real-time updates
              </Badge>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-8 pt-8 text-center">
              <div className="space-y-1">
                ...
                <div className="text-sm text-muted-foreground font-medium">Active Opportunities</div>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <div className="text-sm text-muted-foreground font-medium">New Updates</div>
              </div>
            </div>
          </div>
        </div>
            ...
            <CardContent className="p-4">
              <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  {/* Sort Selection */}
                  <Select value={sortBy} onValueChange={(value: 'newest' | 'deadline' | 'featured') => setSortBy(value)}>
                    <SelectTrigger className="w-48">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h2 className="text-2xl font-semibold text-foreground">
                  {filteredAndSortedOpportunities.length} Opportunit{filteredAndSortedOpportunities.length === 1 ? 'y' : 'ies'} Found
                </h2>
                <p className="text-muted-foreground">
                  {selectedCategory !== 'all' && `Filtered by ${categories.find(c => c.value === selectedCategory)?.label}`}
                  {searchTerm && (selectedCategory !== 'all' ? ' • ' : '') + `Searching for "${searchTerm}"`}
                </p>
              </div>
              
              <div className="text-sm text-muted-foreground">
                Sorted by {sortBy === 'newest' ? 'newest first' : sortBy === 'deadline' ? 'deadline approaching' : 'featured first'}
              </div>
            </div>
