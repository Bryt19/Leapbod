...
import { HiSearch, HiCheck, HiX, HiTrash, HiUser, HiDocumentText, HiClock, HiEye, HiPencil } from 'react-icons/hi'
...
export default function AdminPanel() {
  ...
  const updateOpportunityStatus = async (opportunityId: string, newStatus: 'approved' | 'rejected') => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          category: editForm.category as 'internship' | 'scholarship' | 'competition' | 'event' | 'job' | 'research',
  ...
  const filteredOpportunities = opportunities.filter(opportunity => {
    ...
    const matchesFilter = opportunityFilter === 'all' || opportunity.status === opportunityFilter
    return matchesSearch && matchesFilter
  })
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-muted-foreground">Manage users, opportunities, and view platform statistics</p>
              </div>
              <TabsList>
              ...
              {stats.pendingOpportunities > 0 && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      There are {stats.pendingOpportunities} opportunities that need your review
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <TableBody>
                          {filteredProfiles.map((profile) => (
                            <TableRow key={profile.id}>
                              <TableCell>{profile.full_name || 'Unknown User'}</TableCell>
                        ...
                        <TableBody>
                          {filteredOpportunities.map((opportunity) => (
                            <TableRow key={opportunity.id}>
                              ...
                              <TableCell>
                                <Badge className={getStatusColor(opportunity.status || 'pending')}>
                                  {opportunity.status}
                                </Badge>
                              </TableCell>
                                  ...
                                  {opportunity.status === 'pending' && (
                                    <>
                                      <Button
                                        ...
                                        onClick={() => updateOpportunityStatus(opportunity.id, 'approved')}
                                      ...
                                      <Button
                                        size="sm"
                                        variant="destructive"
                                        onClick={() => updateOpportunityStatus(opportunity.id, 'rejected')}
                                        disabled={updating === opportunity.id}
                                      >
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {dialogMode === 'delete'
                ...
                : isOpportunity(selectedItem)
                ? selectedItem.organization && `Organization: ${selectedItem.organization}`
                : null}
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                {isProfile(selectedItem) && dialogMode === 'view' && (
                  <>
                    ...
                    <div>
                      <strong>Joined:</strong> {format(new Date(selectedItem.created_at || ''), 'MMMM d, yyyy')}
                    </div>
                  </>
                )}
                ...
                {isOpportunity(selectedItem) && dialogMode === 'view' && (
                  <>
                    ...
                    <div>
                      ...
                      <p className="mt-1">{selectedItem.requirements?.join('\n') || 'No requirements specified'}</p>
                    </div>
                    <div>
                      <strong>Benefits:</strong>
                      <p className="mt-1">{selectedItem.benefits?.join('\n') || 'No benefits specified'}</p>
                    </div>
                    <div>
                      <strong>Location:</strong> {selectedItem.location}
                    </div>
                    <div>
                      <strong>Deadline:</strong> {selectedItem.deadline ? format(new Date(selectedItem.deadline), 'MMMM d, yyyy') : 'No deadline'}
                    </div>
                  </>
                )}
                ...
                {isOpportunity(selectedItem) && dialogMode === 'edit' && (
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Textarea
                        name="benefits"
                        value={Array.isArray(editForm.benefits) ? editForm.benefits.join('\n') : editForm.benefits || ''}
                        onChange={handleInputChange}
                        rows={4}
            ...
            {dialogMode === 'delete' && isOpportunity(selectedItem) && (
              <>
                ...
                <Button
                  ...
                  disabled={updating === selectedItem.id}
                >
                  {updating === selectedItem.id ? 'Deleting...' : 'Delete Opportunity'}
                </Button>
              </>
            )}
          </DialogFooter>
